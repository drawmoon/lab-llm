> 环境配置以 Ubuntu24.04 为例

### 如果你使用的是 WSL
部分工具安装过程中可能会使用外网，下面示例是在宿主 Windows 中开启代理，并在 WSL 中设置使用  Windows 的代理：
下面以 fish 为例，在 `.profile.fish` 中添加如下指令：
```sh
set wslip (ip route | grep default | awk '{print $3}')
export HTTP_PROXY="http://$wslip:7897";HTTPS_PROXY="http://$wslip:7897";ALL_PROXY="http://$wslip:7897"
```
如果你使用的是 bash 则将第一行指令改为 `wslip=$(ip route | grep default | awk '{print $3}')`，其他保持不变
### 更换 Apt 源
首先备份原始配置：
```sh
sudo cp /etc/apt/sources.list.d/ubuntu.sources /etc/apt/sources.list.d/ubuntu.sources.bak
```
使用 vim 或其他的文本编辑器添加如下指令到 `/etc/apt/sources.list.d/ubuntu.sources`：
```sh
Types: deb
